...
- ❌ Email notifications - `emitEmailNotification()` is a no-op
- ❌ Email reminder notifications - Disabled in message fetching
- ❌ Email reminders setting - Toggle disabled in Settings page
...
- `src/app/pages/SettingsPage.tsx` - Email settings tab (email reminders disabled)
- `electron/main.ts` - Email service initialization and notification handlers (blocked)
  - `emitEmailNotification()` - No-op function (email notifications disabled)
  - `email:fetchMessages` handler - Email notification code disabled
